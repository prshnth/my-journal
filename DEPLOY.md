...
**12.** Visit **`https://journal.yourdomain.com`** → the login screen asks for your
`DASHBOARD_PASSWORD`. Sign in to use the Journal, Runs, and Todos tabs.

To track a run, use Telegram's Reply action on that morning's running-plan message and send,
for example, `done, 25 min, energy 4/5, pain 0/10`. The journal preserves the full reply and
the Runs tab shows the extracted details. A worker restart can catch up the 7:00 a.m. training
message until noon.
...
The `migrate` service runs all pending files in `drizzle/` before the updated web and worker
containers start, so no separate schema command is needed on the VPS.
...
| Runs tab stays empty | Reply to a running nudge (prefer Telegram's Reply action), then confirm the `migrate` service completed successfully |
