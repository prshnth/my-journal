...
import { getDashboardData, getJournalEntries } from "@/lib/stats";
import { Heatmap } from "@/components/Heatmap";
import { Card, EmptyState, EntryItem } from "@/components/ui";

export const dynamic = "force-dynamic";

function hrefFor(params: { q?: string; limit?: number }): string {
...
export default async function JournalPage({
  ...
  const [data, activity] = await Promise.all([
    getJournalEntries({ search: q, limit }),
    getDashboardData(),
  ]);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card title="journal consistency · last 18 weeks">
        <div className="mb-3 flex flex-wrap gap-3 text-xs text-zinc-500">
          <span>
            {activity.totals.entries} {activity.totals.entries === 1 ? "entry" : "entries"}
          </span>
          <span>{activity.totals.streak}-day streak</span>
        </div>
        <Heatmap days={activity.calendar} />
      </Card>
