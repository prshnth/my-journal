...
function WeeklyBars({ weeks }: { weeks: RunWeek[] }) {
  ...
  return (
    <div className="overflow-x-auto pb-1">
      <div className="flex h-40 min-w-[34rem] items-end gap-2" aria-label="Runs per week for the last 12 weeks">
        {weeks.map((week) => {
          const height = week.runs ? Math.max(12, (week.runs / maxRuns) * 112) : 4;
          return (
            <div key={week.startDate} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-2">
              ...
              <div
                ...
                className={`w-full max-w-8 rounded-t-md ${week.runs ? "bg-amber-500" : "bg-zinc-800"}`}
                style={{ height }}
              />
              <div className="whitespace-nowrap text-[10px] text-zinc-600">{week.label}</div>
            </div>
          );
        })}
      </div>
      ...
      <section className="space-y-3">
        ...
        {data.recent.length === 0 ? (
          ...
          <ul className="space-y-2">
            {data.recent.map((run) => (
              <li key={run.id} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
                ...
                <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-100">{run.text}</p>
              </li>
            ))}
          </ul>
        )}
